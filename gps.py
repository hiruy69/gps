...
HOST = '0.0.0.0'
# socket.gethostbyname(socket.gethostname())  #function may not work in Linux systems, change to string with IP adress example: "192.168.0.1"
PORT = 5151
...
def start_server_trigger():
	...
	with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
		...
		while True:
			...
			conn, addr = s.accept()
			conn.settimeout(320) #connection timeout, change this value to close the socket if no DATA is received for X amount of seconds
			with conn:
